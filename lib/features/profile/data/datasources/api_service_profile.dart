...
class ApiserviceProfile extends ApiBase {
  Future<CodeModel> updateProfile(
      String name, String email, String phone) async {
    return executeAndHandleErrorServer<CodeModel>(() async {
      final response = await patch('$BASE_URL/account', body: {
        'name': name,
        'email': email,
        'phone': phone,
      }, headers: {
  ...
  Future<CodeModel> getProfile() async {
    return executeAndHandleErrorServer<CodeModel>(() async {
      final response = await get('$BASE_URL/account', headers: {
        'Authorization': token ?? "",
      });
      return CodeModel.fromJson(response.json);
    });
